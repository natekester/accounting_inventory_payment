...
func (s *Service) SyncCurrentItems(ctx context.Context) (int, error) {
	...
	if s.qbo != nil {
		...
		if err != nil {
			return 0, fmt.Errorf("QBO pull sync failed: %w", err)
		}
	...
	for _, item := range items {
		if item.QBOItemID == "" && s.qbo != nil {
			qboID, err := s.qbo.CreateItem(ctx, *item)
			if err != nil {
				return 0, fmt.Errorf("QBO push sync failed for SKU %s: %w", item.SKU, err)
			}

			if err := s.repo.UpdateQBOItemID(ctx, item.ID, qboID); err != nil {
				return 0, fmt.Errorf("failed to update local database with synced QBO ID for SKU %s: %w", item.SKU, err)
			}
